...
void main(){
    ...
    for(int i=0;i<4;i++){
        ...
        float denominator=4.0f*max(dot(N,V),0.0f)*max(dot(N,L),0.0f)+0.0001;//0.0001是避免除零错误
...
vec3 fresnelSchlick(float cosTheta,vec3 F0){
    return F0+(1-F0)*pow(clamp(1.0f-cosTheta,0.0f,1.0f),5.0f);
}
